...
interface SettingsModalProps {
    ...
    t: Record<string, any>;
    useGeminiAI: boolean;
    onUseGeminiAIChange: (use: boolean) => void;
}

export const SettingsModal: React.FC<SettingsModalProps> = ({ onClose, theme, onThemeChange, language, onLanguageChange, intensity, onIntensityChange, t, useGeminiAI, onUseGeminiAIChange }) => {
        ...
        <div className="modal-overlay settings-modal" onClick={onClose}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="setting-item" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: '0.75rem' }}>
                 ...
                 <div className="setting-item" title={!isApiKeyConfigured ? "API_KEY not configured. This feature is disabled." : "Toggle generative AI responses"}>
                    <span className="setting-label" style={{ opacity: isApiKeyConfigured ? 1 : 0.5 }}>🤖 Enable Gemini AI</span>
                    <label className="toggle-switch">
                        <input type="checkbox" checked={useGeminiAI} onChange={() => onUseGeminiAIChange(!useGeminiAI)} disabled={!isApiKeyConfigured} />
                        <span className="slider"></span>
                    </label>
                </div>
